...
export default function ChatWindow({ character, sessionId, language = "en", speechLocale = "en-US" }) {
  ...
  function startListening() {
    ...
    // Stores each finalized *segment* by its own result-array index, keyed
    // so a segment can only ever be overwritten, never re-appended. This is
    // the fix for the "who who are who are you" duplication: on Android
    // Chrome especially, the engine sometimes re-fires `onresult` with
    // isFinal:true for the SAME segment index more than once (each time
    // with a slightly more complete transcript of that segment). The old
    // code did `finalTranscriptRef.current += newFinal` on every such
    // firing, so every re-fire piled another copy of the words on top of
    // the previous one. Indexing by segment and overwriting in place makes
    // repeated firings for the same segment idempotent, while still
    // correctly concatenating genuinely separate segments in order.
    const finalSegments = [];
    ...
    recognition.onresult = (event) => {
      ...
      finalTranscriptRef.current = finalSegments.filter(Boolean).join(" ").trim();
